...
// Define parameters to estimate
  // Parameters for model of phenotype
   ...
   // Random effects
   matrix[3,n_ind]         zI; //(intercepts, res_impact, impact for each individual)
   vector<lower=0>[3]      sigma_I; // sd  intercepts, res_impact, impact
   cholesky_factor_corr[3] L;  // factor to estimate covariance int-slopes
   real<lower=0> sigma_e;
 }

 transformed parameters{
    matrix[3,n_ind] I; //  Unscaled blups intercept, res_impact and impact for each ind.
...
real cov_psi_epsilon;
real phi[n_ind];

for (i in 1:n_ind) {
 phi[i]= psi*xi[i] + I[3, individual[i]];
}
...
Sigma2_phi=psi^2*variance(xi) + sigma_I[3]^2;
Sigma2_e=sigma_e^2;

Omega_I = L * L';
cov_int_psi = Omega_I[1,2]*sqrt(Sigma2_psi*Sigma2_intercept);
cov_int_epsilon = Omega_I[1,3]*sqrt(Sigma2_epsilon*Sigma2_intercept);
cov_psi_epsilon = Omega_I[2,3]*sqrt(Sigma2_epsilon*Sigma2_psi);
 }", file="RR.stan")
```
...
stan_func<-function(datalist){
  ...
  psi <- as.data.frame(t(dat[,grepl("^I\\[2,",names(dat))]))
  ...
  dat$cov_psi_phi <- cor(psi$`50%`,phi$`50%`)*sqrt(dat["50%","Sigma2_phi"]*dat["50%","Sigma2_psi"])
  dat <- dat[,c("B_0", "psi", "Sigma2_intercept", "Sigma2_psi",
                "Sigma2_epsilon", "Sigma2_phi","cov_int_psi",
                  "cov_int_epsilon", "cov_psi_epsilon","cov_int_phi",
                "cov_psi_phi", "Sigma2_e", "lp__")]
  }

# Apply function to list of data sets
output <- do.call(rbind,parSapply(cl, datalist, stan_func))
output <- as.data.frame(output)
output$Parameter <- rep(c("B_0", "psi", "Sigma2_intercept", "Sigma2_psi",
                "Sigma2_epsilon", "Sigma2_phi","cov_int_psi",
                  "cov_int_epsilon", "cov_psi_epsilon","cov_int_phi",
                "cov_psi_phi", "Sigma2_e", "lp__"),length(datalist))
