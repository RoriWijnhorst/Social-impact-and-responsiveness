...
# --- 1. 
dfl <- readRDS("Data/dfl_400_50_8_1x.RDS")
...
Sys.time()
write.csv(output, "Output/res_400_50_8_1x_M1.csv", row.names = FALSE)
...
# --- 3. 
dfl <- readRDS("Data/dfl_1600_200_8_1x.RDS")
output_list <- future_lapply(
  dfl, 
  stan_func, 
  future.globals = list(sm = sm),
...
output <- do.call(rbind, output_list)
Sys.time()
write.csv(output, "Output/res_1600_200_8_1x_M1.csv", row.names = FALSE)
...
output <- do.call(rbind, output_list)
Sys.time()
write.csv(output, "Output/res_3200_400_8_1x_M1.csv", row.names = FALSE)

# --- 5. 
dfl <- readRDS("Data/dfl_6400_800_8_1x.RDS")
output_list <- future_lapply(
  dfl, 
  stan_func, 
  future.globals = list(sm = sm),
...
output <- do.call(rbind, output_list)
Sys.time()
write.csv(output, "Output/res_6400_800_8_1x_M1.csv", row.names = FALSE)