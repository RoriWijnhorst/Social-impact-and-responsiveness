...
df2_3200_1600_2_1x  <-read.csv("Output/res_3200_1600_2_1x_M4.csv") 
df2_3200_800_4_1x   <-read.csv("Output/res_3200_800_4_1x_M4.csv") 
df2_3200_400_8_1x   <-read.csv("Output/res_3200_400_8_1x_M4.csv") 
df2_3200_200_16_1x  <-read.csv("Output/res_3200_200_16_1x_M4.csv")
df2_3200_100_32_1x  <-read.csv("Output/res_3200_100_32_1x_M4.csv")

df3_3200_1600_2_1x  <-read.csv("Output/res_3200_1600_2_1x_M5.csv") 
df3_3200_800_4_1x   <-read.csv("Output/res_3200_800_4_1x_M5.csv") 
df3_3200_400_8_1x   <-read.csv("Output/res_3200_400_8_1x_M5.csv") 
df3_3200_200_16_1x  <-read.csv("Output/res_3200_200_16_1x_M5.csv")
df3_3200_100_32_1x  <-read.csv("Output/res_3200_100_32_1x_M5.csv")
...
p1 <- ggplot(summary1, aes(x = as.numeric(ind), y = mean_bias, color = Parameter,  group = Model)) + 
  ...
  scale_y_continuous(name="Bias (%)", limits=c(-30,10), breaks = seq(-30, 10, 5)) +
  scale_colour_manual(values=c("#E5C494"), labels=c("\U1D449\U1D719"),
                      guide="none") +
  scale_shape_discrete(labels=c("I&R (Trait+EIV+RS)", "No random slopes (Trait+EIV)", "No error correction (Trait+RS)"))+
  scale_linetype_discrete(labels=c("I&R (Trait+EIV+RS)", "No random slopes (Trait+EIV)", "No error correction (Trait+RS)"))+
...
p5 <- ggplot(summary3, aes(x = as.numeric(ind), y = mean_bias, color = Parameter,  group = Model)) + 
  ...
  scale_x_continuous(name="number of individuals\nsocial partners", 
                     breaks=c(1:5), labels=c("1600\n2","800\n4","400\n8","200\n16", "100\n32")) +
  scale_y_continuous(name="Bias (%)", limits=c(-55,2), breaks = seq(-55, 2, 5)) +
...
# Create your main patch
patch1 <- ((p1 + p3 + p5) / (p2 + p4 + p6)) + plot_layout(guides = "collect")

# Add titles above each top column
patch1 <- patch1 +
  plot_annotation(
    title = NULL,
    theme = theme(
  ...
  theme(plot.margin = margin(5,5,5,5))  # tidy spacing

# Use wrap_elements + annotation_custom for individual top labels
final_plot <- wrap_elements(panel = patch1) +
  patchwork::plot_annotation(
    tag_levels = NULL
  ) &
  theme(plot.margin = margin(0,0,0,0))

# Manually add titles above top row:
library(grid)
...
title_row <- plot_grid(
  ggdraw() + draw_label(TeX("Population response $\\bar{\\psi}$"),
                        hjust = 0.5, vjust = 0.5, size = 12),
  ggdraw() + draw_label(TeX("Social impact $V_{\\phi}$"),
                        hjust = 0.5, vjust = 0.5, size = 12),
  ggdraw() + draw_label(TeX("Mean-impact $Cov_{\\alpha\\phi}$"),
                        hjust = 0.5, vjust = 0.5, size = 12),
...
# Example: load a Google font
font_add_google("Lato", "lato")

title_row <- plot_grid(
  ggdraw() + draw_label(TeX("Population response $\\bar{\\psi}$"), family="lato", hjust = 0.5),
  ggdraw() + draw_label(TeX("Social impact $V_{\\phi}$"), family="lato", hjust = 0.5),
  ggdraw() + draw_label(TeX("Mean-impact $\\mathrm{Cov}_{\\alpha\\phi}$"), family="lato", hjust = 0.5),
  ncol = 3
)
final_plot
...
df$Model <- factor(df$Model, labels = c("I&R (Trait+EIV+RS)", "No random slopes (Trait+EIV)", "No error correction (Trait+RS)"))
...
latex_output <- paste0(
  ...
  "  \\toprule \n",
  " & Individuals & I\\&R & Trait+EIV & Trait+RS \\\\ \n",
  "  \\midrule\n",
