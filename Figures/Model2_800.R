...
df4_800_400_2_1x  <-read.csv("Output/res_800_400_2_1x_M4.csv")
df4_800_200_4_1x  <-read.csv("Output/res_800_200_4_1x_M4.csv")
df4_800_100_8_1x  <-read.csv("Output/res_800_100_8_1x_M4.csv")
df4_800_50_16_1x  <-read.csv("Output/res_800_50_16_1x_M4.csv")

df5_800_400_2_1x  <-read.csv("Output/res_800_400_2_1x_M5.csv")
df5_800_200_4_1x  <-read.csv("Output/res_800_200_4_1x_M5.csv")
df5_800_100_8_1x  <-read.csv("Output/res_800_100_8_1x_M5.csv")
df5_800_50_16_1x  <-read.csv("Output/res_800_50_16_1x_M5.csv")
...
p1 <- ggplot(summary1, aes(x = as.numeric(ind), y = mean_bias, color = Parameter,  group = Model)) + 
  ...
  scale_y_continuous(name="Bias (%)", limits=c(-20,20), breaks = seq(-20, 20, 5)) +
...
p2<-ggplot(summary1, aes(x = as.numeric(ind), y = RMAD, color = Parameter,  group = Model)) + 
  ...
  scale_x_continuous(name="number of individuals\nsocial partners", 
                     breaks=c(1:4), labels=c("400\n2","200\n4","100\n8","50\n16")) +
  scale_y_continuous(name="Dispersion (%)", limits=c(9,40), breaks = seq(10, 40, 5)) +
  scale_colour_manual(values=c("#E5C494"), labels=c("\U1D449\U1D719"),
                      guide="none") +
  scale_shape_discrete(labels=c("I&R (Trait+EIV+RS)", "No random slopes (Trait+EIV)", "No error correction (Trait+RS)"))+
  scale_linetype_discrete(labels=c("I&R (Trait+EIV+RS)", "No random slopes (Trait+EIV)", "No error correction (Trait+RS)"))+
...
p5 <- ggplot(summary3, aes(x = as.numeric(ind), y = mean_bias, color = Parameter,  group = Model)) + 
  ...
  scale_x_continuous(name="number of individuals\nsocial partners", 
                     breaks=c(1:4), labels=c("400\n2","200\n4","100\n8","50\n16")) +
  scale_y_continuous(name="Bias (%)", limits=c(-60,2), breaks = seq(-60, 2, 5)) +
...
p6<-ggplot(summary3, aes(x = as.numeric(ind), y = RMAD, color = Parameter,  group = Model)) + 
  ...
  scale_x_continuous(name="number of individuals\nsocial partners", 
                     breaks=c(1:4), labels=c("400\n2","200\n4","100\n8","50\n16")) +
  scale_y_continuous(name="Bias (%)", limits=c(9,40), breaks = seq(10, 40, 5)) +
  scale_colour_manual(values=c("#3F4176"), labels=c("\U1D449\U1D719"),
                      guide="none") +
  scale_shape_discrete(labels=c("I&R (Trait+EIV+RS)", "No random slopes (Trait+EIV)", "No error correction (Trait+RS)"))+
  scale_linetype_discrete(labels=c("I&R (Trait+EIV+RS)", "No random slopes (Trait+EIV)", "No error correction (Trait+RS)"))+
